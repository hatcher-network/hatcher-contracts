 ...
 *
 * Each user has a Collection, user upload DataItem to his Collection
 * A `Collection` is a `bucket` resource and an `DataItem` is an `object` resource
 * A DataItem must be put into a Collection
 * A Collection can contain DataItems
 * Anyone must create a Collection before creating an DataItem
 *
 * And an DataItem should be bonding to a group (usage right for this data item)
 * Only members of the group can get the DataItem
 *
 */
